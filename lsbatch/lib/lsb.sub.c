...
char *loginShell;
static char *additionEsubInfo;

extern const char *defaultSpoolDir;
static const char* getDefaultSpoolDir(void);
...
xdrSubReqSize(struct submitReq *req)
{
    ...
    sz += ALIGNWORD_(strlen(req->queue) + 1) + 4 +
        ...
        + ALIGNWORD_(strlen(req->job_description) + 1) + sizeof(int);
...
void startNios(struct submitReq *submitReq, int asock, LS_LONG_INT jobId)
{
    ...
    sprintf(niosPath, "%s/nios", niosParams[0].paramValue);
    niosArgv[0] = niosPath;
    niosArgv[1] = "-N";
    niosArgv[2] = sockStr;
...
#define SET_PARM_NUMBER(name, field, delnum, defnum) {          \
    ...
    sprintf (esub, "%s/%s", lsbParams[LSB_SERVERDIR].paramValue, "esub");
    if (stat(esub, &sbuf) < 0)
        return 0;
...
readOptFile(char *filename, char *childLine)
{
    ...
    if ((fp = fopen(filename, "r")) == NULL) {
        ls_syslog(LOG_ERR, "%s failed open %s: %m", __func__, filename);
        return -1;
    }
