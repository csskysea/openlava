...
static struct jData *find_first_pend_job(struct qData *);
static char *has_license_in_rusage(struct jData *);
static bool_t is_using_license(const char *, struct jData *);
static bool_t is_preemptable_resource(const char *);
static bool_t is_pend_for_slot(struct jData *);
static void print_preempted_hosts(struct jData *);
...
%s: No jobs to trigger preemption in queue %s",
    ...
    while (jPtr) {
         ...
         * than slots
         */
        jPtr->wants_license = NULL;
        if (mbdParams->preemptableResources) {
            if ((jPtr->wants_license = has_license_in_rusage(jPtr)) == NULL) {
                if (logclass & LC_PREEMPT) {
                    ls_syslog(LOG_INFO, "\
%s: job %s queue %s is not pending for any resource %s", __func__,
            ...
            if (! is_pend_for_slot(jPtr)) {
                if (logclass & LC_PREEMPT) {
                    ls_syslog(LOG_INFO, "\
%s: job %s queue %s is not pending for slots", __func__,
                              lsb_jobid2str(jPtr->jobId), qPtr->queue);
                }
                goto dalsi;
            }
            if (LINK_NUM_ENTRIES(jPtr->preempted_hosts) > 0) {
                if (logclass & LC_PREEMPT) {
                    ls_syslog(LOG_INFO, "\
%s: job %s queue %s already triggered preemption", __func__,
                              lsb_jobid2str(jPtr->jobId),
                              qPtr->queue);
                    print_preempted_hosts(jPtr);
                }
                goto dalsi;
            }
        }
...
%s: job %s queue %s trying to harvest %d slots in queue %s",
            ...
            for (jPtr2 = jDataList[SJL]->forw;
                ...
                if (mbdParams->preemptableResources) {
                    ...
                    strcpy(jbuf, lsb_jobid2str(jPtr->jobId));
                    /* Is the job using license?
                     */
                    if (! is_using_license(jPtr->wants_license, jPtr2)) {
                        if (logclass & LC_PREEMPT) {
                            ls_syslog(LOG_INFO, "\
%s: job %s queue %s is not using license %s the preemptor %s wants", __func__,
                                      lsb_jobid2str(jPtr2->jobId),
                                      qPtr->queue, jPtr->wants_license,
                                      jbuf);
                        }
                    }
                    continue;
...
%s: job %s gives up %d slots got %d want %d", __func__,
                ...
                /* Save the hosts only if we preempt for slots
                 */
                if (mbdParams->preempt_slot_suspend) {
...
%s: job %s preempting exec hosts %s", __func__, lsb_jobid2str(jPtr->jobId),
                        ...
                        /* jPtr is the job that is preempting
                         * jPtr2 is the job that is being preempted
                         */
...
/* has_license_in_rusage()
 */
static char *
has_license_in_rusage(struct jData *jPtr)
{
    ...
    while ((r = traverse_link(&iter))) {
        ...
        for (cc = 0; cc < allLsInfo->nRes; cc++) {
            ...
            if (is_preemptable_resource(allLsInfo->resTable[cc].name))
                return allLsInfo->resTable[cc].name;
        }
    }
 ...
 */
static bool_t
is_using_license(const char *lic_name, struct jData *jPtr)
{
    struct resVal *resPtr;
    int cc;
    int rusage;
    ...
    while ((r = traverse_link(&iter))) {
        ...
        for (cc = 0; cc < allLsInfo->nRes; cc++) {
            ...
            if (is_preemptable_resource(allLsInfo->resTable[cc].name)
                && strcmp(lic_name, allLsInfo->resTable[cc].name) == 0) {
                return true;
            }
        }
    }
...
/* print_preempted_hosts()
 */
static void
print_preempted_hosts(struct jData *jPtr)
{
    linkiter_t iter;
    struct hData *hPtr;
    ...
    while ((hPtr = traverse_link(&iter))) {
        ls_syslog(LOG_INFO, "\
%s: job %s preempted host %s", __func__, lsb_jobid2str(jPtr->jobId), hPtr->host);
    }
}
