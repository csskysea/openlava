            ...
            int actFlags, int actPeriod, logType logFlag)
{
    ...
    if (logclass & (LC_TRACE | LC_SIGNAL))
        ls_syslog(LOG_INFO, "\
%s: signal %s job %s with actFlags %d; reasons %x, subresons %d, sigValue %d",
             ...
             int reasons, int subReasons, logType logFlag)
{
    ...
    if (logclass &  LC_SIGNAL)
        ls_syslog(LOG_INFO, "\
%s: signal %s job %s reasons %x subresons %d, sigValue %d",
                  __func__, getLsbSigSymbol(sig_decode(sigValue)),
               ...
               int jState, int reasons, int subReasons, logType logFlag)
{
    ...
    if (logclass &  LC_SIGNAL)
        ls_syslog(LOG_INFO, "\
%s: signal %s job %s with reasons %x subresons %d, sigValue %d",
                  __func__, getLsbSigSymbol (sig_decode(sigValue)),
                  lsb_jobid2str(jp->jobSpecs.jobId),
                  jp->jobSpecs.reasons,
                  jp->jobSpecs.subreasons, sigValue);
               ...
               logType logFlag)
{
    ...
    if (logclass &  LC_SIGNAL)
        ls_syslog(LOG_DEBUG, "\
%s: job %s jobSpecs.reasons %x sigValue %d suspendReasons %x", __func__,
                  lsb_jobid2str(jp->jobSpecs.jobId), jp->jobSpecs.reasons,
                  sigValue, suspendReasons);
...
jobsig(struct jobCard *jp, int sig, int forkSig)
{
    ...
    if (logclass & (LC_TRACE | LC_SCHED | LC_EXEC)) {
        ls_syslog(LOG_INFO, "\
%s: Send signal %d to job %s", __func__, sig, lsb_jobid2str(jp->jobSpecs.jobId));
    }
...
mykillpg(struct jobCard *jp, int sig)
{
    ...
    if (logclass & LC_SIGNAL)
        ls_syslog(LOG_INFO, "\
%s: job %s pid %d pgid %d resstartpid %d sig %d", __func__,
    ...
    if (jp->runRusage.npgids > 0 && !(jp->regOpFlag & REG_RUSAGE)) {
        ...
        for (i = 0; i < jp->runRusage.npgids; i++) {
            if (pgkillit(jp->runRusage.pgid[i], sig) < 0) {
                ...
                if (logclass & LC_SIGNAL)
                    ls_syslog(LOG_INFO, "%\
s: Job %s pgkillit(%d) are gone", __func__, lsb_jobid2str(jp->jobSpecs.jobId),
                              jp->runRusage.pgid[i]);
            }
        }
        if (ndead == jp->runRusage.npgids) {
            if (logclass & LC_SIGNAL)
                ls_syslog(LOG_INFO, "%s: Job %s pgids all dead", __func__,
                          lsb_jobid2str(jp->jobSpecs.jobId));
            return -1;
        }
    } else {
        if (pgkillit(jp->jobSpecs.jobPGid, sig) < 0) {
            if (logclass & LC_SIGNAL)
                ls_syslog(LOG_INFO, "%s: Job %s pgkillit(%d) is gone", __func__,
                          lsb_jobid2str(jp->jobSpecs.jobId),
                          jp->jobSpecs.jobPGid);
            return -1;
        }
...
jobact(struct jobCard *jp, int actValue, char *actCmd, int actFlags, int forkSig)
{
    ...
    if (logclass & (LC_TRACE | LC_SCHED | LC_EXEC))
        ls_syslog(LOG_INFO, "\
%s: Send batch system signal %d to job %s", __func__, actValue,
                  lsb_jobid2str(jp->jobSpecs.jobId));
