...
replay_newstat(char *filename, int lineNum)
{
    ...
    if ((IS_POST_DONE(newStat->jStatus))||(IS_POST_ERR(newStat->jStatus))) {
        /* Sometime the event JOB_STAT_DONE is missing before the JOB_STAT_PDONE. Add JOB_STAT_DONE event */
        if (IS_POST_DONE(newStat->jStatus) && !IS_FINISH(jp->jStatus)) {
            updHostLeftRusageMem(jp, 1);
            jp->endTime = newStat->endTime;
            jp->cpuTime = newStat->cpuTime;
            jp->exitStatus = JOB_STAT_DONE;
...
log_job_preemption(struct jData *jPtr, link_t *rl)
{
    ...
    /* Traverse the list of jobs that are being
     * preempted by jPtr.
     */
...
%s: reading line %d in file %s: %s", __func__, lineNum,
        ...
        switch (logPtr->type) {
            ...
            case EVENT_DEL_JGRP:
                ...
                break;
            case EVENT_JOB_PREEMPTING:
                jobId = LSB_JOBID(logPtr->eventLog.jobPreempting.jobid,
                                  logPtr->eventLog.jobPreempting.idx);
                break;
            case EVENT_JOB_PREEMPTED:
                jobId = LSB_JOBID(logPtr->eventLog.jobPreempted.jobid,
                                  logPtr->eventLog.jobPreempted.idx);
                break;
            default:
                break;
        }
